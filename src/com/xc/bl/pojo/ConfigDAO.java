...
public class ConfigDAO extends BaseHibernateDAO {
	...
	public Config findById(java.lang.Integer id) {
		...
		try {
			Config instance = (Config) getSession().get(
					"com.xc.bl.pojo.Config", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Config instance) {
		...
		try {
			List results = getSession().createCriteria("com.xc.bl.pojo.Config")
	...
	public List findByCfgName(Object cfgName) {
		return findByProperty(CFG_NAME, cfgName);
	}

	public List findByCfgValue(Object cfgValue) {
		return findByProperty(CFG_VALUE, cfgValue);
	}
